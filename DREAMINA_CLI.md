...
> VIP-модели скрыты в `--help`, но CLI их принимает для **всех команд** (text2video, multimodal2video, image2video и др.). VIP даёт только приоритет в очереди — функциональность та же. Нужен VIP-аккаунт.
